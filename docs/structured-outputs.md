...
- **Type safety**: Use with Python type hints for end-to-end safety
...
2. **Add the outputFormat parameter**: Include the `output_format` parameter in your query options with `type: "json_schema"` and your schema definition.
...
async for msg in query(
    ...
    if hasattr(msg, 'subtype'):
        if msg.subtype == 'success' and hasattr(msg, 'structured_output'):
