...
The `hooks` option is a dictionary where:
...
1. **Input data** (`dict`): Event details. See input data section for fields
2. **Tool use ID** (`str | None`): Correlate `PreToolUse` and `PostToolUse` events
3. **Context** (`HookContext`): Reserved for future use in Python

### Input data

The first argument to your hook callback contains information about the event. Field names use snake_case.
...
| `tool_input` | `object` | Arguments passed to the tool | PreToolUse, PostToolUse |
...
| `additionalContext` | `string` | PostToolUse, UserPromptSubmit | Context added to the conversation |
...
Hooks can perform async operations like HTTP requests. Handle errors gracefully by catching exceptions instead of throwing them:
